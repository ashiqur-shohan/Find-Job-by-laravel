<x-layout>
    ...
    <div class="bg-white mx-auto p-8 rounded-lg shadow-md w-full md:max-w-3xl">
        ...
        <form method="POST" action="{{ route('jobs.update', $job->id) }}" enctype="multipart/form-data">
            ...
            <x-inputs.text-area id="descripion" name="description" label="Description"
            ...
            <x-inputs.text-area id="benefits" name="benefits" label="Benefits"
                placeholder="Health insurance, 401k, paid time off" rows="2" :value="old('benefits',$job->title)" />
            ...
            <x-inputs.text id="company_website" name="company_website" type="url" label="Company Website"
                placeholder="Company Website" :value="old('company_website',$job->company_website)" />

            <x-inputs.text id="contact_phone" name="contact_phone" label="Contact Phone" placeholder="Contact Phone" :value="old('contact_phone',$job->contact_phone)" />

            <x-inputs.text id="contact_email" name="contact_email" label="Contact Email" placeholder="Contact Email"
                type="email" :value="old('email',$job->email)" />
